...
def convert2hw(
    ...
    if model_type == "lstm":
        model_params.update({"batch_size": subset_size})
    ...
    if model_type == "lstm":
        network_design = model.sequential.create_design(model_params["name"])
    else:
    ...
    firmware = GetFirmware(network_design=network_design)
